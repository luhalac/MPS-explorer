...
class MPS_explorer(QtGui.QMainWindow):
    ...
    def create_scatter_plot(self, plot, x, y, brush, size=1):
        scatter_item = pg.ScatterPlotItem(x, y, pen=None, brush=brush, size=size)
        plot.addItem(scatter_item)

    def create_histogram(self, plot, data, brush, pen, bins=500):
        hist, bin_edges = np.histogram(data, bins=bins)
        width = np.mean(np.diff(bin_edges))
        bincenters = np.mean(np.vstack([bin_edges[:-1], bin_edges[1:]]), axis=0)
        bar_graph = pg.BarGraphItem(x=bincenters, height=hist, width=width, brush=brush, pen=pen)
        plot.addItem(bar_graph)

    def display_scatterplot(self, x, y, z, brush, pen, scatter_layout, hist_layout, filename=''):
        scatter_widget = pg.GraphicsLayoutWidget()
        scatter_plot = scatter_widget.addPlot(title=f"Scatter plot (x, y) - {filename}")

        scatter_item = pg.ScatterPlotItem(x, y, pen=None, brush=brush, size=1)
        scatter_plot.addItem(scatter_item)

        xmin, xmax = np.min(x), np.max(x)
        ymin, ymax = np.min(y), np.max(y)
        scatter_plot.setLabels(bottom=('x [nm]'), left=('y [nm]'))
        scatter_plot.setAspectLocked(True)
        scatter_plot.setXRange(xmin, xmax, padding=0)
        scatter_plot.setYRange(ymin, ymax, padding=0)

        scatter_layout.addWidget(scatter_widget)

        npixels = np.size(x)
        roi_pos = (int(min(x)), int(min(y)))
        roi_extent = int(npixels / 10)

        roi_pen = pg.mkPen(color='b')
        roi = pg.ROI(roi_pos, roi_extent, pen=roi_pen)
        roi.setZValue(10)
        roi.addScaleHandle([1, 1], [0, 0])
        roi.addRotateHandle([0, 0], [1, 1])
        scatter_plot.addItem(roi)

        hist_widget = pg.GraphicsLayoutWidget()
        hist_plot = hist_widget.addPlot(title=f"z Histogram - {filename}")

        self.create_histogram(hist_plot, z, brush, pen)
    ...
    def scatterplot(self):
        ...
        self.display_scatterplot(xdata, ydata, zdata, self.brush1, self.pen1, self.ui.scatterlayout, self.ui.zhistlayoutch1, filename1)

        filename2 = self.ui.lineEdit_filename_2.text()
        if filename2:
            xdata2, ydata2, zdata2 = self.import_file_ch2(filename2)
            self.display_scatterplot(xdata2, ydata2, zdata2, self.brush2, self.pen2, self.ui.scatterlayout, self.ui.zhistlayoutch2, filename2)
